...
import { Calendar, CheckCircle2, Dot, Eye, HandHelping, Info, Lock, Luggage, Save, Shield, UploadCloud, User } from 'lucide-react';
...
import MissingFieldsModal, { formatLabel } from '@/components/missing-fields-modal';
...
import { EDUCATION_OPTIONS, LEAD_SOURCE_OPTIONS, RESIDENT_STATUS, WEEK_DAYS } from '@/lib/content/careers-config';
...
const DEFAULT_VALUES: CareerApplicationFormData = {
    ...
    availability: WEEK_DAYS.map((day) => ({ week_day: day, time_from: '', time_to: '' })),
    resume_file: null,
    ...
    education: '',
    skills: [],
    references: [{ full_name: '', position: '', work: '', email: '', phone: '' }],
};
...
function FileUploadField({ label, accept = '.pdf,.doc,.docx', required = false, value, onChange, error }: FileUploadFieldProps) {
    ...
    return (
        <div className="flex flex-col gap-2">
            ...
            >
                ...
                <span className="text-muted-foreground">{value ? value.name : 'Click to upload or drag and drop'}</span>
                <span className="mt-1 text-xs text-muted-foreground">Accepted formats: {accept}</span>
                <input
                    id={inputId}
                    type="file"
                    className="hidden"
 ...
 */
export default function CareerApplicationForm({ careers, preselectedCareer }: CareerApplicationFormProps) {
    const { data, setData, post, processing, errors, reset, transform } = useForm<CareerApplicationFormData>(DEFAULT_VALUES);
    ...
    const [savedDraft, setSavedDraft] = useState<Partial<CareerApplicationFormData> | null>(null);
    const [emailWarning, setEmailWarning] = useState<string | null>(null);
    const [checkingEmail, setCheckingEmail] = useState(false);

    const cookieConsentAllowed = () => localStorage.getItem('cookieConsent') !== 'declined';
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(() => {
            ...
            fetch(`/check-email?email=${encodeURIComponent(data.email)}`)
                .then((res) => res.json())
                .then((json: { exists?: boolean }) => setEmailWarning(json.exists ? 'Email is already in use' : null))
    ...
    // Progress bar / missing-fields tracking over the required-fields list.
    useEffect(() => {
        const { progress: pct, missingFields: missing } = computeFormProgress(data, REQUIRED_FIELDS);
        setProgress(pct);
    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => {
            const { resume_file: _resumeFile, cover_letter_file: _coverLetterFile, ...rest } = data;
    ...
    const updateAvailability = (idx: number, field: 'time_from' | 'time_to', value: string) => {
        setData(
            'availability',
            data.availability.map((slot, i) => (i === idx ? { ...slot, [field]: value } : slot)),
        );
    };

    const updateReference = (idx: number, field: keyof ReferenceEntry, value: string) => {
        setData(
            'references',
            data.references.map((ref, i) => (i === idx ? { ...ref, [field]: value } : ref)),
        );
    };

    const addReference = () => {
        setData('references', [...data.references, { full_name: '', position: '', work: '', email: '', phone: '' }]);
    };
    ...
    const submitApplication = () => {
        post('/careers/apply', {
            ...
            preserveScroll: true,
            onSuccess: (page) => {
                const flash = (page.props as { flash?: { application?: { reference_number?: string } } }).flash;
                setReferenceNumber(flash?.application?.reference_number ?? null);
                localStorage.removeItem(DRAFT_STORAGE_KEY);
    ...
    return (
        <>
            <div className="my-10 sticky top-16 rounded-lg bg-white p-6 shadow-2xl">
                <div className="mb-4 flex flex-col items-start justify-between gap-2 sm:flex-row sm:items-center sm:gap-0">
                    <p className="flex items-center gap-2 text-sm font-medium text-primary sm:text-base">
                        <CheckCircle2 className="h-5 w-5" /> Application Progress
                    </p>
                    <p className="text-sm font-semibold text-primary sm:text-base">{progress}%</p>
                </div>
            ...
            {/* Applicant Info */}
            <Accordion type="single" collapsible className="my-10 rounded-lg bg-white shadow-2xl">
                <AccordionItem value="applicant-info">
                    <AccordionTrigger className="flex items-center gap-3 rounded-t-lg bg-gradient-to-br from-secondary-orange/10 to-transparent p-4 transition hover:bg-secondary-orange/20 sm:p-5">
                        <div className="flex flex-row items-center gap-3">
                            ...
                            <p className="text-sm font-medium text-primary sm:text-base">Applicant Information</p>
                        </div>
                    </AccordionTrigger>
                    <AccordionContent className="p-5">
                        ...
                        <div className="mt-6 grid grid-cols-1 gap-6 md:grid-cols-2">
                            ...
                            <div className="flex flex-col">
                                ...
                                <Input
                                    ...
                                    className="mt-2 rounded-[10px]"
                                />
                                {checkingEmail && <p className="text-sm text-gray-500">Checking...</p>}
                        ...
                        <div className="mt-2">
                            <Label>
                                Residential Status <span className="text-red-700">*</span>
                            </Label>
                            <Select value={data.resident_status} onValueChange={(value) => setData('resident_status', value)}>
                            ...
                            {errors.resident_status && <p className="text-sm text-red-600">{errors.resident_status}</p>}
                        </div>
                ...
                <AccordionItem value="position-details">
                    <AccordionTrigger className="flex items-center gap-3 rounded-t-lg bg-gradient-to-br from-secondary-orange/10 to-transparent p-4 transition hover:bg-secondary-orange/20 sm:p-5">
                        <div className="flex items-center gap-3">
                            ...
                            <p className="text-sm font-medium text-primary sm:text-base">Position Details</p>
                        </div>
                    </AccordionTrigger>
                    <AccordionContent className="p-5">
                            ...
                            <div>
                                ...
                                <Select
                                    ...
                                    onValueChange={(value) => {
                                        ...
                                        const selected = careers.find((c) => c.position === value);
                                ...
                                >
                                    ...
                                    <SelectContent>
                                        {careers.map((career) => (
                                            <SelectItem key={career.id} value={career.position}>
                                                {career.position}
                                            </SelectItem>
                                        ))}
                                    </SelectContent>
                                </Select>
                                {errors.position_applied && <p className="text-sm text-red-600">{errors.position_applied}</p>}
                            </div>

                            <div>
                                <Label>
                                    Profession status <span className="text-red-700">*</span>
                                </Label>
                                <Select value={data.profession_status} onValueChange={(value) => setData('profession_status', value)}>
                                    <SelectTrigger className="mt-2 rounded-[10px]">
                                        <SelectValue placeholder="Select status" />
                                    </SelectTrigger>
                                    <SelectContent>
                                        {PROFESSION_STATUS_OPTIONS.map((option) => (
                                ...
                                {errors.profession_status && <p className="text-sm text-red-600">{errors.profession_status}</p>}
                            </div>
                        </div>
                    </AccordionContent>
                </AccordionItem>
                ...
                <AccordionItem value="availability">
                    <AccordionTrigger className="flex items-center gap-3 rounded-t-lg bg-gradient-to-br from-secondary-orange/10 to-transparent p-4 transition hover:bg-secondary-orange/20 sm:p-5">
                        <div className="flex items-center gap-3">
                            ...
                            <p className="text-sm font-medium text-primary sm:text-base">Availability</p>
                        </div>
                    </AccordionTrigger>
                    <AccordionContent className="p-5">
                        <p className="mb-4">Please indicate your available hours for each day of the week.</p>

                        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                            {data.availability.map((slot, idx) => (
                                <div key={slot.week_day} className="flex flex-col gap-2">
                                    <Label>{slot.week_day}</Label>
                                    <div className="flex gap-2">
                                        <Input
                                            type="time"
                                            value={slot.time_from}
                                            onChange={(e) => updateAvailability(idx, 'time_from', e.target.value)}
                                            className="rounded-[10px]"
                                        />
                                        <Input
                                            type="time"
                                            value={slot.time_to}
                                            onChange={(e) => updateAvailability(idx, 'time_to', e.target.value)}
                                            className="rounded-[10px]"
                                        />
                                    </div>
                                </div>
                        ...
                        <div className="mt-4">
                            <Label>
                                Preferred Start Date <span className="text-red-700">*</span>
                            </Label>
                            <Input
                                type="date"
                                value={data.preferred_start_date}
                                onChange={(e) => setData('preferred_start_date', e.target.value)}
                                className="mt-2 rounded-[10px]"
                                min={new Date().toISOString().split('T')[0]}
                            />
                            {errors.preferred_start_date && <p className="text-sm text-red-600">{errors.preferred_start_date}</p>}
                        </div>

                        <div className="mt-4">
                            <Label>
                                Are you currently working with other contractors or companies?{' '}
                                <span className="text-red-700">*</span>
                            </Label>
                            <RadioGroup
                                value={data.is_working_with_other ? 'yes' : 'no'}
                                onValueChange={(v) => setData('is_working_with_other', v === 'yes')}
                            >
                ...
                <AccordionItem value="personal-info">
                    <AccordionTrigger className="flex items-center gap-3 rounded-t-lg bg-gradient-to-br from-secondary-orange/10 to-transparent p-4 transition hover:bg-secondary-orange/20 sm:p-5">
                        <div className="flex items-center gap-3">
                            ...
                            <p className="text-sm font-medium text-primary sm:text-base">Personal Information</p>
                        </div>
                    </AccordionTrigger>
                    <AccordionContent className="p-5">
                        <div className="mb-4">
                            <Label>
                                Highest Level of Education <span className="text-red-700">*</span>
                            </Label>
                            <Select value={data.education} onValueChange={(value) => setData('education', value)}>
                                <SelectTrigger className="mt-2 rounded-[10px]">
                        ...
                        <div className="mb-4">
                            <EnterInput inputs={data.skills} setInputs={(skills) => setData('skills', skills)} error={errors.skills} title="Skills" />
                        </div>
                        ...
                        <div className="mb-4">
                            <FileUploadField
                                ...
                                onChange={(file) => setData('resume_file', file)}
                        ...
                        <div className="mb-4">
                            <FileUploadField
                                ...
                                onChange={(file) => setData('cover_letter_file', file)}
                        ...
                        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                            <div>
                                ...
                                <RadioGroup
                                    ...
                                    onValueChange={(v) => setData('drivers_license', v === 'yes')}
                                >
                                    <div className="mt-2 flex flex-row gap-6">
                                        <div className="flex items-center gap-2">
                            ...
                            <div>
                                ...
                                <RadioGroup
                                    ...
                                    onValueChange={(v) => setData('has_vehicle', v === 'yes')}
                                >
                                    <div className="mt-2 flex flex-row gap-6">
                                        <div className="flex items-center gap-2">
                        ...
                        <div className="mt-6">
                            <h3 className="mb-4 font-medium text-primary">
                                References <span className="text-xs text-muted-foreground">(at least 1 reference is required)</span>
                            </h3>

                            {data.references.map((reference, idx) => (
                                <div key={idx} className="relative mb-4 rounded-lg border border-gray-200 p-4">
                                    <p className="mb-2 font-semibold">Reference {idx + 1}</p>

                                    <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
                                        <div>
                                            <Label>Full Name {idx === 0 ? '*' : ' '}</Label>
                                            <Input
                                                value={reference.full_name}
                                                onChange={(e) => updateReference(idx, 'full_name', e.target.value)}
                                                className="mt-2 rounded-[10px]"
                                            />
                                            {errors[`references.${idx}.full_name`] && (
                                                <p className="text-sm text-red-600">{errors[`references.${idx}.full_name`]}</p>
                                            )}
                                        </div>

                                        <div>
                                            <Label>Position {idx === 0 ? '*' : ' '}</Label>
                                            <Input
                                                value={reference.position}
                                                onChange={(e) => updateReference(idx, 'position', e.target.value)}
                                                className="mt-2 rounded-[10px]"
                                            />
                                            {errors[`references.${idx}.position`] && (
                                                <p className="text-sm text-red-600">{errors[`references.${idx}.position`]}</p>
                                            )}
                                        </div>

                                        <div>
                                            <Label>Work {idx === 0 ? '*' : ' '}</Label>
                                            <Input
                                                value={reference.work}
                                                onChange={(e) => updateReference(idx, 'work', e.target.value)}
                                                className="mt-2 rounded-[10px]"
                                            />
                                            {errors[`references.${idx}.work`] && (
                                                <p className="text-sm text-red-600">{errors[`references.${idx}.work`]}</p>
                                            )}
                                        </div>
                                        ...
                                        <div>
                                            ...
                                            <Input
                                                ...
                                                className="mt-2 rounded-[10px]"
                                            />
                                            {errors[`references.${idx}.email`] && (
                                                <p className="text-sm text-red-600">{errors[`references.${idx}.email`]}</p>
                                            )}
                                        </div>

                                        <div>
                                            <Label>Phone {idx === 0 ? '*' : ' '}</Label>
                                            <Input
                                                value={reference.phone}
                                                onChange={(e) => updateReference(idx, 'phone', e.target.value.replace(/[^0-9()+-\s]/g, ''))}
                                                className="mt-2 rounded-[10px]"
                                                maxLength={12}
                                            />
                                            {errors[`references.${idx}.phone`] && (
                                                <p className="text-sm text-red-600">{errors[`references.${idx}.phone`]}</p>
                                            )}
                                        </div>
                                    </div>
                ...
                <AccordionItem value="additional-info">
                    <AccordionTrigger className="flex items-center gap-3 rounded-t-lg bg-gradient-to-br from-secondary-orange/10 to-transparent p-4 transition hover:bg-secondary-orange/20 sm:p-5">
                        <div className="flex items-center gap-3">
                            ...
                            <p className="text-sm font-medium text-primary sm:text-base">Additional Information</p>
                        </div>
                    </AccordionTrigger>
                    <AccordionContent className="p-5">
                        <div>
                        ...
                        <div className="mt-1">
                            ...
                            <Select
                                ...
                                onValueChange={(value) => {
                                    ...
                                    setData('lead_source', value !== 'other' ? value : '');
                                }}
                            >
                                <SelectTrigger className="mt-2 rounded-[10px]">
                            ...
                            {errors.lead_source && <p className="text-sm text-red-600">{errors.lead_source}</p>}
                        </div>
                        ...
                        <div className="mt-1">
                            <Label>
                                Why would you like to work with Creative Abilities Therapy Services?{' '}
                                <span className="text-red-700">*</span>
                            </Label>
                            <textarea
                                ...
                                rows={4}
                            />
                            {errors.reason_for_applying && <p className="text-sm text-red-600">{errors.reason_for_applying}</p>}
                        </div>
                ...
                <AccordionItem value="privacy-consent">
                    <AccordionTrigger className="flex items-center gap-3 rounded-t-lg bg-gradient-to-br from-secondary-orange/10 to-transparent p-4 transition hover:bg-secondary-orange/20 sm:p-5">
                        <div className="flex items-center gap-3">
                            ...
                            <p className="text-sm font-medium text-primary sm:text-base">Privacy &amp; Consent</p>
                        </div>
                    </AccordionTrigger>
                    <AccordionContent className="p-5">
                        <p className="mb-4">
                            Your privacy is important to us. Please review and accept our data handling practices.
                        </p>
                        ...
                        <div className="m-5 rounded-sm border bg-white p-5 shadow-sm">
                            <p className="flex flex-row items-center gap-2 text-base font-medium">
                                <Info className="h-5 w-5 text-primary" /> Information We Collect
                            </p>
                            <div className="mt-4 space-y-2">
                                <p className="flex flex-row flex-wrap items-center gap-2 text-sm">
                                    <Dot className="mt-1 text-primary" />
                                    <span className="font-bold">Personal Information:</span> Name, contact details, and address
                                </p>
                                <p className="flex flex-row flex-wrap items-center gap-2 text-sm">
                                    <Dot className="mt-1 text-primary" />
                                    <span className="font-bold">Professional Documents:</span> Resume, cover letter, and
                                    certifications
                                </p>
                                <p className="flex flex-row flex-wrap items-center gap-2 text-sm">
                                    <Dot className="mt-1 text-primary" />
                                    <span className="font-bold">Application Details:</span> Position preferences, availability,
                                    and qualifications
                                </p>
                                <p className="flex flex-row flex-wrap items-center gap-2 text-sm">
                                    <Dot className="mt-1 text-primary" />
                                    <span className="font-bold">Additional Information:</span> References and other supporting
                        ...
                        <div className="m-5 rounded-sm border p-5">
                            ...
                            <p className="mt-2 flex flex-row items-center gap-2 text-sm">
                                <Dot className="text-primary" /> To evaluate your qualifications for the position
                            </p>
                            <p className="mt-2 flex flex-row items-center gap-2 text-sm">
                                <Dot className="text-primary" /> To communicate with you about your application status
                            </p>
                            <p className="mt-2 flex flex-row items-center gap-2 text-sm">
                                <Dot className="text-primary" /> To conduct reference and background checks (if applicable)
                            </p>
                            <p className="mt-2 flex flex-row items-center gap-2 text-sm">
                                <Dot className="text-primary" /> To comply with legal and regulatory requirements
                            </p>
                        </div>
                        ...
                        <div className="m-5 rounded-sm border bg-white p-5 shadow-sm">
                            ...
                            <div className="mt-4 space-y-2">
                                <p className="flex flex-row flex-wrap items-center gap-2 text-sm">
                                    <Dot className="mt-1 text-primary" />
                                    <span className="font-bold">Retention Period:</span> Applications are kept for 12 months
                                    for future opportunities
                                </p>
                                <p className="flex flex-row flex-wrap items-center gap-2 text-sm">
                                    <Dot className="mt-1 text-primary" />
                                    <span className="font-bold">Access:</span> Only authorized HR personnel and hiring
                                    managers have access
                                </p>
                                <p className="flex flex-row flex-wrap items-center gap-2 text-sm">
                                    <Dot className="mt-1 text-primary" />
                                    <span className="font-bold">Security:</span> All data is encrypted and stored securely
                                </p>
                                <p className="flex flex-row flex-wrap items-center gap-2 text-sm">
                                    <Dot className="mt-1 text-primary" />
                                    <span className="font-bold">Your Rights:</span> You may request deletion of your data at
                                    any time
                                </p>
                            </div>
                        </div>
                        ...
                        <div className="m-5 flex flex-col gap-5 rounded-sm border-2 border-primary/50 bg-secondary-orange/10 p-5 sm:flex-row">
                            <div className="flex flex-shrink-0 items-start gap-2 sm:items-center">
                                ...
                                />
                                <label htmlFor="consent" className="font-bold text-primary">
                            ...
                            <div className="flex flex-1 flex-col gap-2 text-sm">
                                ...
                                <p className="flex gap-2">
                                    <Dot className="mt-1 flex-shrink-0 text-primary" />
                                    I consent to Creative Abilities Therapy Services collecting, using, and storing my
                                    personal information as described above
                                </p>
                                <p className="flex gap-2">
                                    <Dot className="mt-1 flex-shrink-0 text-primary" />
                                    I understand my application will be retained for 12 months
                                </p>
                                <p className="flex gap-2">
                                    <Dot className="mt-1 flex-shrink-0 text-primary" />
                                    I have read and agree to the{' '}
                                    <a href="/termsandconditions" target="_blank" rel="noopener noreferrer" className="text-primary">
                                        Terms and Conditions
                                    </a>{' '}
                                    and{' '}
                                    <a href="/privacypolicy" target="_blank" rel="noopener noreferrer" className="text-primary">
                                        Privacy Policy
                                    </a>
                                </p>
                                <p className="flex gap-2">
                        ...
                        <div className="flex flex-col items-center justify-center">
                            ...
                            <p className="mt-3 text-center text-sm text-muted-foreground">
                                ...
                                <a href="mailto:privacy@creativeabilitiestherapy.com" className="text-primary">
                        ...
                        <p className="mt-10 text-center text-sm text-muted-foreground">
                            Please accept the consent agreement above to enable submission
                        </p>
                    </AccordionContent>
                </AccordionItem>
            </Accordion>
            ...
            <ApplicationSubmittedModal isOpen={submittedOpen} onClose={() => setSubmittedOpen(false)} referenceNumber={referenceNumber} />

            <MissingFieldsModal open={missingDialogOpen} onOpenChange={setMissingDialogOpen} missingFields={missingFields} />

            <Dialog open={showLoadDraftModal} onOpenChange={(open) => !open && setShowLoadDraftModal(false)}>
                <DialogContent className="max-w-md p-6">
                    <DialogHeader>
                        <DialogTitle>Unfinished Application Found</DialogTitle>
                    </DialogHeader>
                    <div className="my-4">You have a saved draft of an application. Do you want to load it?</div>
